...
from picks_manager.views import ManageDB, pre_fetch_decorator
...
def timer(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        print('Funciton {} takes {:.2f} ms'.format(
            func.__name__,
            duration
        ))
        return result
...
class TestWinRatesCollection(TestCase):
    ...
    def test_updating_the_database(self):
        """ Just updating but timer only measures the actual important operation to time. Rest is executed like once in 4 months """
        scanned_games = ManageDB().update_map_list_and_winrate(25, debug = True)
        print(scanned_games)
    ...
    def test_synergies(self):
        ...
        winning_team = [{'tag': '#PU00QLLV0', 'name': 'ズラタンイブラヒモビッチ', 'brawler': {'id': 16000085, 'name': 'KENJI', 'power': 11, 'trophies': 1014}}, {'tag': '#2Y9QRCQL', 'name': '겨링겨링', 'brawler': {
            'id': 16000084, 'name': 'MOE', 'power': 11, 'trophies': 1030}}, {'tag': '#8CL998GVY', 'name': 'みず💧', 'brawler': {'id': 16000038, 'name': 'PAM', 'power': 11, 'trophies': 925}}]
        losing_team = [{'tag': '#PRQULYG9J', 'name': 'Thə Łúcifer惡😈', 'brawler': {'id': 16000025, 'name': 'TICK', 'power': 11, 'trophies': 991}}, {'tag': '#YYYYJUP9', 'name': '鬥士', 'brawler': {
            'id': 16000085, 'name': 'EDGAR', 'power': 11, 'trophies': 1029}}, {'tag': '#P2L2LUVPV', 'name': 'Q娃', 'brawler': {'id': 16000020, 'name': 'FRANK', 'power': 11, 'trophies': 1011}}]
